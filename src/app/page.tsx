...
export default function Home() {
  ...
  /* main search (AI first with timeout, classic fallback) */
  const runSearch = async () => {
    ...
    try {
      ...
      // 1) AI interpreter (cap at 7s). Only try if user typed something.
      if (q.length > 0) {
        const ai = new URLSearchParams({ q });
        if (lat !== null && lng !== null) {
          ai.set("lat", String(lat));
          ai.set("lng", String(lng));
        }
        try {
          const data = await fetchJSONWithTimeout<{ results?: Place[] }>(
            `/api/ai/search?` + ai.toString(),
            7000
          );
          ...
          // swallow AI errors/timeouts and fall back to classic
        }
      }

      // 2) Classic fallback (cap at 8s), using minimal chips -> filters
      ...
      // pass text as keyword to help ranking a bit

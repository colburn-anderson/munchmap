  ...
  try {
    ...
    if (GOOGLE_MAPS_API_KEY) {
      const url =
        ...
        `?query=${encodeURIComponent(q)}&key=${GOOGLE_MAPS_API_KEY}`;
    ...
    // Demo fallback so the UI still shows something
    return NextResponse.json({
      ok: true,
      query: q,
      results: [{ place_id: "demo-1", name: `Demo: ${q}`, formatted_address: "—", rating: 4.6 }],
    });
  } catch (e: any) {
